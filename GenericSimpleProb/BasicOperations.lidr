...
> |||
> toList : {A, B : Type} -> SimpleProb A B -> List (A, B)
> toList (MkSimpleProb aps _) = aps


> |||
> normalize : {A, B : Type} -> DecEq B => SimpleProb A B -> SimpleProb A B
> normalize {A} (MkSimpleProb Nil sum) = MkSimpleProb Nil sum
> normalize {A} (MkSimpleProb (ap :: Nil) sum) = MkSimpleProb (ap :: Nil) sum
> normalize {A} {B} (MkSimpleProb (ap :: ap' :: aps) sum) = MkSimpleProb aps' sum' where
>   aps' : List (A, B)
