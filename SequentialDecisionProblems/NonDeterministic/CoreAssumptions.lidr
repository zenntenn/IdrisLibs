> module SequentialDecisionProblems.NonDeterministic.CoreAssumptions
...
sequential decision problems presented in |CoreTheoryNonDeterministic|.
...
|FullTheoryNonDeterministic|. The additional assumptions needed to
implement the full theory are summarized in
|ExtraAssumptionsNonDeterministic|.

Both here and in |ExtraAssumptionsNonDeterministic|, the assumptions are
...
state and selected control. But in a non-deterministic decision process,
the consequences of selecting a control in a given state are uncertain.
Here, the decision maker knows what *can* happen, but not what *will*
acrually happen.
...
> rewards t x y = map (reward t x y) (nexts t x y)
...
all "next" states satisfy certain properties. This requires |List| to be
a "container":
...
>                (a : A) -> (as : List A) -> All P as -> a `Elem` as -> P a
