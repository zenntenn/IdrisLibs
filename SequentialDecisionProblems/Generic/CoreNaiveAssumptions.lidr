> module SequentialDecisionProblems.Generic.CoreNaiveAssumptions
...
< nexts : (t : Nat) -> (x : State t) -> (y : Ctrl t x) -> State (S t)
...
|nexts| to a monadic transition function:
...
>                               (a : A) -> (ma : M A) ->
>                               a `Elem` ma -> CoreNaiveAssumptions.NotEmpty ma

> postulate elemNotEmptySpec1 : {A : Type} ->
>                               (ma : M A) -> CoreNaiveAssumptions.NotEmpty ma ->
>                               Sigma A (\ a => a `Elem` ma)

In "CoreNaiveTheory.lidr" we are going to implement a generic form (for
all |State : (t : Nat) -> Type|, |Ctrl : (t : Nat) -> (x : State t) ->
Type|, |M : Type -> Type|, etc.) of the backwards induction algorithm
originally proposed by Bellman in 1957 [2].
...
t x| that maximises an arbitrary function from |Ctrl t x| to
|Nat|. Thus, we assume that users that want to apply the theory are able
to implement two functions

> argmax : {t : Nat} ->
>          (x : State t) -> 
>          (f : Ctrl t x -> Val) -> Ctrl t x
...
< maxSpec : {t : Nat} ->
<           (x : State t) -> 
<           (f : Ctrl t x -> Val) -> (y : Ctrl t x) ->
<           (f y) `LTE` (max x v f)

As it turns out, |argmaxSpec| and |maxSpec| are only needed to implement
the full theory. We introduced them explicitly in
|FullNaiveAssumptions|.
