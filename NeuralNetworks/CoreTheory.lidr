> module NeuralNetworks.CoreTheory
...
A neural network is a layered acyclic graph with an input layer, an
output layer and finite number of *hidden* layers

> nHiddenLayers : Nat

Thus, the total number of layers is |2 + nHiddenLayers = S (S
nHiddenLayers)|

> nLayers : Nat
> nLayers = S (S nHiddenLayers)
...
At each layer we have a finite number of nodes

> nNodes : Layer -> Nat

All nodes in a layer have the same type. Different layers might have
nodes of different types

> Node : Layer -> Type

The nodes of the network have a bias and an activation function:

> bias : {l : Layer} -> Node l -> Double

> aFun : {l : Layer} -> Node l -> Double -> Double

The activation function of the nodes of the output layer is required to
be the identity function:

> aFunSpec : (n : Node oLayer) -> aFun n = id

** Edges

In each layer, nodes are connected to nodes of the next layer through
weighted edges:
...
> network : (l : Layer) -> Node l -> Node (S l) -> Weight 
